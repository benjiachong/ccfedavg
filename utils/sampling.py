...
def mnist_noniid_multi2(dataset, num_users):
    """
    每个节点不再是任选两个类，而是指定两个类
    Sample non-I.I.D client data from CIFAR10 dataset
    :param dataset:
    :param num_users:
    :return:
    """
    num_shards, num_imgs = num_users*2, len(dataset)//(2*num_users)
    idx_shard = [i for i in range(num_shards)]
    ...
    # 组合idx_shard and assign
    for i in range(num_users):
        rand_set = [idx_shard.pop(0),idx_shard.pop(-1)]
...
def mnist_noniid_multi3(dataset, num_users):
    """
    前10%个节点不再是任选两个类，而是指定两个类（cifar_noniid_multi2模式），
    其他节点任选两类（cifar_noniid_multi模式）
    Sample non-I.I.D client data from CIFAR10 dataset
    :param dataset:
    :param num_users:
    :return:
    ...
    # 组合idx_shard and assign
    part = 0.1
    for i in range(int(part*num_users)):
        rand_set = [idx_shard.pop(0),idx_shard.pop(-1)]
    ...
    for i in range(int(part*num_users), num_users):
        rand_set = set(np.random.choice(idx_shard, num_shards//num_users, replace=False))
        idx_shard = list(set(idx_shard) - rand_set)
...
def cifar_noniid(dataset, num_users, iidpart = 0.1, class_num=10):
    """
    Sample non-I.I.D client data from CIFAR10 dataset
    :param dataset:
    :param num_users:
    ...
    for i in range(class_num):
        iid_idx = np.hstack((iid_idx, idxs[num_shards * num_imgs//class_num*i : num_shards * num_imgs//class_num*i + iid_num//class_num]))
        noniid_idx = np.hstack(
            (noniid_idx, idxs[num_shards * num_imgs // class_num * i + iid_num//class_num : num_shards * num_imgs // class_num * (i+1)]))
    np.random.shuffle(iid_idx)
...
def cifar_noniid_multi2(dataset, num_users):
    """
    每个节点不再是任选两个类，而是指定两个类
    Sample non-I.I.D client data from CIFAR10 dataset
    :param dataset:
...
def cifar_noniid_multi3(dataset, num_users):
    """
    前10%个节点不再是任选两个类，而是指定两个类（cifar_noniid_multi2模式），
    其他节点任选两类（cifar_noniid_multi模式）
    Sample non-I.I.D client data from CIFAR10 dataset
